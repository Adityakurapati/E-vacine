...
public class DataBase {
	...
	public int insertIntoDB(ArrayList e) {
		...
		try {
			...
			sql = "INSERT into registration1 VALUES(sequence05.nextval,?,?,?,?,?,?,?,?,?)";
	...
	public int login(String pa , String adhar ) {
		...
		try {
			...
			sql = "select  PASSWORD ,FNAME  from registration1 where AADHAR = ? ";
			stm = con.prepareStatement(sql);
	...
	{		
		...
		try {
			...
			String sql = "select FNAME ,LNAME,EMAIL,PHONE,GENDER, ADDRESS , DOB ,PASSWORD from registration1 where AADHAR = ? ";
			stm = con.prepareStatement(sql);
			stm.setString(1,adhar);
			rs= stm.executeQuery();
	...
	public int UpdateDetails(ArrayList e , String Adhar)
	{
		...
		try {
			...
			String sql = "UPDATE registration1 SET FNAME = ? , LNAME = ? ,PHONE = ? ,EMAIL = ? , DOB = ? , ADDRESS = ?   where AADHAR = ? ";
			stm = con.prepareStatement(sql);
	...
	public int forgotPass(String Adhar, String pass1)
	{
		...
		try {
			...
			String sql = "UPDATE registration1 SET PASSWORD = ?  where AADHAR = ? ";
			stm = con.prepareStatement(sql);
	...
	public   int bookingUpdate(String Adhar,String Vaccine , String date ,String center )
	{
		...
		try {
			...
			sql = "INSERT into CovidVaccinabooking(VACCINE,FIRST_DOSE,FIRST_DOSE_DATE,CENTER, AADHAR) VALUES(?,?,?,?,?) ";
			    ...
			    {
			    	JOptionPane.showMessageDialog(null, "YOU CANT BOOK 2ND VACCINE Until 4 Days ARE COMPLETE FROM YOUR 1ST VACCINE ");
			    }
